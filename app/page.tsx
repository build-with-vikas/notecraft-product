...
export default function Page() {
  ...
  return <main className={`nc-shell mode-${mode}`} style={{ '--scroll-progress': scrollProgress, '--hero-progress': heroProgress, '--pointer-x': pointer.x, '--pointer-y': pointer.y } as React.CSSProperties}>
    <header className="nc-nav"><a href="#top" className="nc-brand"><span><Sparkles size={16} /></span> NOTECRAFT</a><nav className={mobileMenu ? 'nc-navlinks open' : 'nc-navlinks'}><a href="#studio">Studio</a><a href="#tools">Tools</a><a href="#method">Method</a></nav><div className="mode-switcher" aria-label="Display mode"><button className={mode === 'light' ? 'active' : ''} onClick={() => setMode('light')}>Light</button><button className={mode === 'dark' ? 'active' : ''} onClick={() => setMode('dark')}>Dark</button><button className={mode === 'auto' ? 'active' : ''} onClick={() => setMode('auto')}>Auto</button></div><button className="nc-menu" onClick={() => setMobileMenu(!mobileMenu)} aria-label="Open navigation"><Menu size={18} /></button><button className="nc-nav-cta" onClick={() => inputRef.current?.click()}>Open files <ArrowRight size={14} /></button></header>
