...
export default function Page() {
  ...
  const [mergeSelection, setMergeSelection] = useState('1-3')
      ...
      if (tool === 'pipeline') {
        ...
        for (let index = 0; index < pipeline.length; index++) {
          ...
          if (step === 'jpg') {
            ...
            const output = await PDFDocument.create(); const selectedFiles = parsePageSelection(mergeSelection, current.length).map((index) => current[index]).filter(Boolean); for (const file of (selectedFiles.length ? selectedFiles : current)) { const source = await PDFDocument.load(await file.arrayBuffer()); const pages = await output.copyPages(source, source.getPageIndices()); pages.forEach((page) => output.addPage(page)) }; const bytes = await output.save(); current = [new File([bytes as BlobPart], 'arranged-merged.pdf', { type: 'application/pdf' })]
          } else {
            const source = await PDFDocument.load(await current[0].arrayBuffer()); const splitFiles: File[] = []; const selectedPages = parsePageSelection(splitRange, source.getPageCount()); for (const pageIndex of (selectedPages.length ? selectedPages : source.getPageIndices())) { const part = await PDFDocument.create(); const [page] = await part.copyPages(source, [pageIndex]); part.addPage(page); const bytes = await part.save(); splitFiles.push(new File([bytes as BlobPart], `arranged-page-${pageIndex + 1}.pdf`, { type: 'application/pdf' })) } current = splitFiles
          }
        }
  ...
  return <main className="nc-shell" style={{ '--scroll-progress': scrollProgress, '--hero-progress': heroProgress, '--pointer-x': pointer.x, '--pointer-y': pointer.y } as React.CSSProperties}>
