      ...
      if (tool === 'pipeline') {
        ...
        for (let index = 0; index < pipeline.length; index++) {
          ...
          if (step === 'jpg') {
            ...
            const source = await PDFDocument.load(await current[0].arrayBuffer()); const splitFiles: File[] = []; for (let pageIndex = 0; pageIndex < source.getPageCount(); pageIndex++) { const part = await PDFDocument.create(); const [page] = await part.copyPages(source, [pageIndex]); part.addPage(page); const bytes = await part.save(); splitFiles.push(new File([bytes as BlobPart], `arranged-page-${pageIndex + 1}.pdf`, { type: 'application/pdf' })) } current = splitFiles
          }
        }
